...
public class TreeUtil {
    ...
    public static <T, R, U> List<T> buildAscTree(List<T> source, Function<T, R> idFunc, Function<T, R> pIdFunc, BiConsumer<T, List<T>> setChildListFunc, BiConsumer<T, U> setHasChildFunc, Predicate<T> rootPIdValue, Comparator<T> sortValue) {
        ...
        List<T> tree = buildTree(source, idFunc, pIdFunc, setChildListFunc, setHasChildFunc, rootPIdValue);
        return tree;
    }
    ...
    public static <T, R> void traverseTree(List<T> source, Function<T, List<T>> getChildListFunc, Function<T, R> getPropertyFunc, BiConsumer<T, R> setPropertyFunc) {
        source.forEach(model -> {
            setPropertyFunc.accept(model, (R) getPropertyFunc.apply(model));
            List<T> childList = getChildListFunc.apply(model);
            if (!ObjectUtil.isEmpty(childList)) {
    ...
    public static <T, R> List<T> searchTree(List<T> source, Function<T, List<T>> getChildListFunc, BiConsumer<T, List<T>> setChildListFunc, Function<T, R> getPropertyFunc, String keyWord) {
        ...
        source.forEach(model -> {
            R r = (R) getPropertyFunc.apply(model);
    ...
    public static <T, R> List<T> findChildList(List<T> source, Function<T, R> idFunc, Function<T, R> pIdFunc, Predicate<R> rootPIdValue) {
        ...
        List<T> list = map2.entrySet().stream().filter(t -> rootPIdValue.test(t.getKey())).map(Map.Entry::getValue).findAny().orElse(new ArrayList<>());
        return list;
    }
